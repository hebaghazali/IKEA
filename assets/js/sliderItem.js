...
onSnapshot(collection(fireStore , 'ProductCategories') , (SnapChanges)=>{
    ...
    var sliderElement = document.getElementById("slider-items");
...
function ShowData (element)
{
    ...
    var sliderElement = document.getElementById("slider-items");
        var row = `
            <ul>
            <li onclick="FillElemData(('${element.id}'))"><a>${elementData.Name}</a></li>
            </ul> `
...
{
    window.open ("../../pages/category.html" , "_self")
    ...
    ItemResult.forEach((doc)=>{
            ...
            var SubSliderItem = document.getElementById("subSliderItems");
            var row = `

            <p>${doc.data().Name}</p> `
        
            SubSliderItem.innerHTML+=row;
